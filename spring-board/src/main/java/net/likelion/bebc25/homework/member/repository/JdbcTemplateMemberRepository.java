...
public class JdbcTemplateMemberRepository implements MemberRepository {
  ...
  public void save(MemberDto member) {
    // 실습 영역
    jdbcTemplate.update("INSERT INTO member (name, password, email) values (?,?,?)"
            , member.getUsername()
            , member.getPassword()
            , member.getEmail()
    );
  ...
  public MemberDto findByUsername(String username) {
    return jdbcTemplate.queryForObject("SELECT * FROM member WHERE name = ?", memberRowMapper, username);
  }
  ...
  public void update(MemberDto member) {
    // 실습 영역
    jdbcTemplate.update("UPDATE member SET name = ?, password = ?, email = ? WHERE id = ?"
            , member.getUsername()
            , member.getPassword()
            , member.getEmail()
            , member.getId());
